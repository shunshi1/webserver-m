...
WebServer::WebServer(
    ...
    assert(srcDir_);
    strncat(srcDir_, "/resources/", 16); // 把src所指字符串的前n个字符添加到dest结尾处(覆盖dest结尾处的'\0')并添加'\0'。
    HttpConn::userCount = 0; // 初始化
    HttpConn::srcDir = srcDir_; // 初始化
...
void WebServer::InitEventMode_(int trigMode) {
    listenEvent_ = EPOLLRDHUP;  // 检测是否正常关闭
...
bool WebServer::InitSocket_() {
    ...
    addr.sin_addr.s_addr = htonl(INADDR_ANY); // 转换
    ...
    SetFdNonblock(listenFd_); // 设置监听非阻塞
